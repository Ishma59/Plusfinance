...
# from django.utils.encoding import smart_str from dateutil.relativedelta import *
...
#from  micro_admin.wrapper import ProfitAndLossReport
...
#class ProfitAndLossReportView(generic.TemplateView):
 #   template_name = "profit_and_loss_report.html"
  ...
  #      now = timezone.now()
   ...
   #         month=(now.month - ((now.month - 1) % 3)),
    ...
    #    end = start + relativedelta(months=3)
     ...
     #   report = ProfitAndLossReportView(start=start, end=end)
      #  report.generate()
...
def create_client_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return JsonResponse({"error": True, "errors": form.errors})
    return render(request, "client/create.html", {'form': form, 'branches': branches, 'client_roles': CLIENT_ROLES })
...
#def income(request):
    #income_list = Receipts.objects.values('entrance_amount', 'membershipfee_amount', 'bookfee_amount',)
...
def day_book_function(request, date):
    ...
    #Total income
    total_income = sum([value for value in income_dict.values()])
    ...
    #total expenses
    total_expenses = sum([value for value in expenses_dict.values()])
    ...
    profitslosssummary  = total_income - total_expenses
...
def ProfitAndLossSummary(object):
    grouping_date = None
    #income_amount = D('0')
    #expenses_amount = D('0')
    ...
    #total expenses
    expenses_amount = D(sum([value for value in expenses_dict.values()]))
      #income
    

    #Total income
    income_amount = D(sum([value for value in income_dict.values()]))
...
def general_ledger_pdf_download(request):
    general_ledger_list = general_ledger
    ...
    try:
        ...
        #return HttpResponse(rendered_html, content_type='application/pdf')

    except Exception as err:
        errmsg = "%s" % (err)
        return HttpResponse(errmsg)

def profitandloss_pdf_download(request, date):
    date = request.GET.get("date")

    try:
        template = get_template("pdf_profitandloss.html")
        context = dict({'pagesize' : 'A4', "mediaroot": settings.MEDIA_ROOT,
            })
        html = template.render(context)
        pdfkit.from_string(html, 'out.pdf')
        pdf=open("out.pdf")
...
def daybook_pdf_download(request, date):
    ...
    receipts_list, total_payments, travellingallowance_list, \
        loans_list, paymentofsalary_list, printingcharges_list, \
        stationarycharges_list, othercharges_list, savingswithdrawal_list,\
        recurringwithdrawal_list, selected_date, fixedwithdrawal_list, total, \
        dict_payments, total_dict,  grouped_receipts_list, \
        thrift_deposit_sum_list, loanprinciple_amount_sum_list, \
        loaninterest_amount_sum_list, entrancefee_amount_sum_list, \
        membershipfee_amount_sum_list, bookfee_amount_sum_list, \
        loanprocessingfee_amount_sum_list, insurance_amount_sum_list, \
    ...
    try:
        ...
        pdf=open("out.pdf")
        response = HttpResponse(pdf.read(), content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename=daybook.pdf'
        pdf.close()
        os.remove("out.pdf")
        return response
        #result = StringIO.StringIO()
